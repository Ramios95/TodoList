...
app.use(cors(corsOptions))
    ...
    app.listen(config.port, function() {
      console.log(
        "Attention please! Server is now listening on port " +
          config.port
      );
      ...
      app.post("/newtask", function(req, res) {
        ...
        task = new Task(id,req.body.title,req.body.description,false,date,date2,date2);
        tab.push(task);
    ...
    app.delete("/deletetask/:taskid",function(req, res)
    {
     ...
     function convertDate(date) {
        ...
        d2= d1.substring(8,10)+'/'+d1.substring(5,7)+'/'+d1.substring(0,4);
        return d2;
    };
